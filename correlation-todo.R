...
(sum((examData2$Exam - mean(examData2$Exam)) * (examData2$Anxiety - mean(examData2$Anxiety)))) / (102 * (sd(examData2$Exam) * sd(examData2$Anxiety)))
cov(examData2$Exam, examData2$Anxiety)

cov(examData2$Exam, examData2$Anxiety) / (sd(examData2$Exam) * sd(examData2$Anxiety))
...
# time: 한 주(week)에 고양이가 집을 떠나서 보낸 시간, 단위는 시간(hour)
...
## TODO: time 과 gender 를 피어슨 상관분석을 수행하고 신뢰구간 추정과 유의성을 검정하라.
...
cor(catData$time, catData$recode)
...
pc <- ggm::pcor(c("Exam", "Anxiety", "Revise"), var(examData2));pc
pc^2
